...
def scoring_prob(actions: pd.DataFrame, l: int = N, w: int = M) -> np.ndarray:
    ...
    print(shot_actions)
    goals = shot_actions[(shot_actions.outcome == '1')]
    print(goals)
...
def move_transition_matrix(
    ...
    X["outcome"] = move_actions.outcome
    ...
    for i in range(0, w * l):
        vc2 = X[
            ((X.start_cell == i) & (X.outcome == "success"))
